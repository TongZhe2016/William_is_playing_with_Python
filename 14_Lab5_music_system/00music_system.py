...
def updateMusicSummary():
    ...
    if len(music_data) == 0:
        ...
        if type(instrument) == str:
            #instrument
            summary = summary + ", instrument = " + str(instrument)
        if type(instrument) == int:
            summary = summary + ", instrument = " + music.instrument_list[instrument]
        #####
        #
        # TODO:
...
def changeInstrument():
    global instrument
    ins_menu = "0 - acoustic grand piano"+\
               "\n10 - music box"+\
               "\n19 - church organ"+\
               ...
               "\n123 - bird tweet"
    # Ask the user for the music file
    ins_name = turtle.numinput("Change Instrument", ins_menu + \
                   "\nPlease enter the instrument number(0-127):")
    if ins_name == None:
        return
    if not(int(ins_name) in [10,19,24,32,40,56,64,73,123]):
        return
    
    music.setinstrument(int(ins_name))
    instrument = music.instrument_list[int(ins_name)]
    updateMusicSummary()
    #####
    #
    # TODO:
...
def transpose():
    global music_data
    trans_name = turtle.numinput("Change Transpose", "Please enter the transposition:")
    if trans_name == None:
        return
    trans_name = int(trans_name)
    for tone in music_data:
        if tone[1] + trans_name > 127:
            tone[1] = 127
        elif tone[1] + trans_name < 0:
            tone[1] = 0
        else:
            tone[1] = tone[1] + trans_name
    #####
    #
    # TODO:
...
def adjustSpeed():
    global music_data
    speed_name = turtle.numinput("Adjust speed", "Please enter the new speed, in percentage:")
    if speed_name == None:
        return
    speed_name = int(speed_name)
    for tone in music_data:
        tone[0] = tone[0]/(speed_name/100)
        tone[2] = tone[2]/(speed_name/100)
    updateMusicSummary()
    #####
    #
    # TODO:
...
def makeCrazyMusic():
    ...
    num = turtle.numinput("Number of Repetitions","Please enter the number of repetitions:")
    if num == None:
        return
    num = int(num)
    dur = turtle.numinput("Duration of the Sequence","Please enter the duration of the sequence:")
    if dur == None:
        return
    dur = float(dur)
    spi = turtle.numinput("Starting Pitch of the Sequence","Please enter the starting pitch of the sequence:")
    if spi == None:
        return
    spi = int(spi)
    epi = turtle.numinput("Ending Pitch of the Sequence","Please enter the ending pitch of the sequence:")
    if epi == None:
        return
    epi = int(epi)
    music_data = []
    if spi >= epi:
        each_time = dur/(spi - epi + 1)
        a = 0
        for _ in range(num):
            for i in range(spi,epi-1,-1):
                tone = [a, i, each_time]
                music_data.append(tone)
                a = a + each_time
    else:
        each_time = dur/(epi - spi + 1)
        a = 0
        for _ in range(num):
